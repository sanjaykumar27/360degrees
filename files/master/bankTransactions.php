...
<?php
  ...
  function getStudentDetailsSql($instsessassocid, $format)
  {
      //$instsessassocid = $_SESSION['instsessassocid'];
      switch ($format) {
          // sql for report display(PDF creation) without pagination
...
// sql for creating csv for bank upload
  ...
  function getDueFeeStructure()
  {
      ...
      foreach ($studentDetails as $key => $value) {
          ...
          $totalFeeInstallments .= " HAVING t2.duedate >= ' $rangeStartDate '";
  ...
  function uploadData($filename)
  {
      ...
      foreach ($finalArray as $key => $value) {
          if (isset($value[9]) && !empty($value[9])) {
              ...
              $intAbbrevArray[] = $value[5];
          }
      }
      $studentDetails = studentDetailsSql($scholarNoArray, $intAbbrevArray);
  ...
  function studentDetailsSql($scholarNoArray, $intAbbrevArray)
  {
      foreach ($scholarNoArray as $key => $value) {
          $sql = "SELECT t1.studentid, t1.scholarnumber, t1.instsessassocid, t2.clsecassocid, t1.firstname, 
                  t1.middlename, t1.lastname,t11.sessionname
                ...
                `tblstudentdetails` AS t12


                WHERE t1.scholarnumber = '$value'
                AND t10.instituteabbrevation = '$intAbbrevArray[$key]'
  ...
  function feeCollectionSQL($studentData = null, $feeDetailsArray)
  {
      ...
      foreach ($feeDetailsArray as $key => $value) {
          ...
          $icid = $value[0];
          $scholar_no = $studentData[$key]['scholarnumber'];
          $mobile = $value[1];
         ...
         $collectionMode  = getMasterId($value[22], "master");
          // Make Collection Type as globally DEFINE in config file

          $collectionType = Inst_Collection_Type;
